...
interface HolidayData {
    date: string;
    name: string;
    type: 'holiday' | 'workday'; // holiday: 节假日, workday: 调休工作日
}

export class HolidayService {
    private cache: Map<string, HolidayData[]> = new Map();
    private readonly API_URL = 'http://timor.tech/api/holiday/year/';
     ...
     * 获取指定年份的节假日数据
     */
    public async getHolidayData(year: number): Promise<HolidayData[]> {
        // 确保已初始化
     ...
     * 带重试机制的数据获取（仅在初始化时使用）
     */
    private async fetchHolidayDataWithRetry(year: number, maxRetries: number): Promise<HolidayData[]> {
     ...
     * @returns 是否为调休工作日
     */
    public async isWorkday(date: Date): Promise<boolean> {
        const year = date.getFullYear();
        const holidayData = await this.getHolidayData(year);
        const dateStr = this.formatDate(date);
     ...
     * @returns 节假日日期数组
     */
    public async getHolidays(year: number): Promise<string[]> {
        const holidayData = await this.getHolidayData(year);
        return holidayData
     ...
     * @returns 调休工作日日期数组
     */
    public async getWorkdays(year: number): Promise<string[]> {
        const holidayData = await this.getHolidayData(year);
        return holidayData
    ...
    private async fetchHolidayData(year: number): Promise<HolidayData[]> {
     ...
     * 更新节假日缓存（手动触发）
     */
    public async updateCache(year: number): Promise<void> {
